...
\begin{center}
    ...
    {\small \href{https://github.com/thomatrixting/Solar_estructure_modeling}{github del proyecto}}
\end{center}
...
Los datos utilizados provienen del Modelo S de Jørgen Christensen-Dalsgaard et al.\ \cite{ChristensenDalsgaard1996}, disponibles públicamente en \url{https://users-phys.au.dk/jcd/solar_models/} bajo la opción ``Limited set of variables for model S''. El conjunto de datos contiene un perfil radial del Sol compuesto por 2482 capas, con las siguientes variables: radio normalizado $r/R_\odot$, velocidad del sonido $c$, densidad $\rho$, presión $P$, exponente adiabático $\Gamma_1$ y temperatura $T$. Todas las constantes físicas empleadas en los cálculos (constante gravitacional $G$, constante de Boltzmann $k_B$, radio y masa solares $R_\odot$ y $M_\odot$, masa del protón $m_p$, constante de Stefan-Boltzmann $\sigma$ y velocidad de la luz $c$) se obtuvieron de la librería \texttt{astropy}, la cual implementa los valores recomendados por CODATA 2014 \cite{mohrCODATARecommendedValues2016}.
...
\end{equation}
función que es monótona creciente en $x\in(0,1)$ y tiene por tanto solución única para cada $K>0$, la cual se resuelve numéricamente con el método de Brent (\texttt{scipy.optimize.brentq}).
...
Se cargaron los datos del Modelo S \cite{ChristensenDalsgaard1996} con 2482 capas radiales, invirtiendo el orden para situar el centro del Sol en el índice cero. Las diferencias de radio entre capas consecutivas se obtuvieron con \texttt{pandas.DataFrame.diff()}, y la masa encerrada $M(r)$ se calculó como la suma acumulada de $\Delta M_i = 4\pi r_i^2\,\rho_i\,\Delta r_i$ (ecuaciones 1 y 2), usando la densidad en unidades SI obtenida tras la conversión de las unidades originales del modelo (CGS).

La figura \ref{fig:masa} muestra la masa encerrada $M(r)$ en función del radio relativo $r/R_\odot$. Se observa que aproximadamente el $90\%$ de la masa solar se concentra por debajo de $r/R_\odot \approx 0{,}3$, lo que refleja el perfil de densidad fuertemente centralizado del Modelo S.
